...
function getToken() {
  if (_jwtToken && Date.now() < _jwtExpiry - 60000) {
    return Promise.resolve(_jwtToken);
  }
  return fetch(API_AUTH, { method: 'GET', credentials: 'omit' })
    .then(function(r) {
      if (!r.ok) throw new Error('Auth échouée (' + r.status + ')');
      return r.json();
    })
    .then(function(d) {
      if (!d || !d.token) throw new Error('Token absent de la réponse.');
      _jwtToken = d.token;
...
function loadRegions(){
  authFetch(API_CHANNELS,{method:'GET'})
    .then(r=>r.json())
...
function loadRegion(id){
  ...
  var _res;
  authFetch(API_CHANNELS+'?id='+encodeURIComponent(id),{method:'GET'})
...
function loadSource(idx){
  ...
  setStatus('load','Chargement...');
  var _res;
  authFetch(API_PROXY+'?url='+encodeURIComponent(src.url),{method:'GET'})
    .then(r=>{_res=r;return r.json();})
...
function addSource(){
  ...
  if(curSrcType==='url'){
    ...
    var _res;
    authFetch(API_PROXY+'?url='+encodeURIComponent(url),{method:'GET'})
      .then(r=>{_res=r;return r.json();})
    ...
    var _res2;
    authFetch(API_PROXY,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({content:fileContent})})
